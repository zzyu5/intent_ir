...
def lower_intent_to_cuda_gpu_kernel(
    ...
    **_: object,
    ...
    if dropout_v1 is not None:
        ...
        launch_override = {"block": [256, 1, 1], "grid": [1, 1, 1]}
        # Fast inclusive scan within a single CTA.
        shared_global_sym = f"__intentir_sh_{_mlir_ident(kernel_name)}_f32"
        shared_global_memref_ty = "memref<256xf32, 3>"
        ...
        lines.append("      %is_tid0 = arith.cmpi eq, %tid, %c0 : index")
        lines.append("      scf.if %is_tid0 {")
        lines.append("        memref.store %neg_inf, %sh[%cCarry] : memref<256xf32, 3>")
        lines.append("      }")
        lines.append("      gpu.barrier")
        ...
        lines.append("        %col = arith.addi %t, %tid : index")
        lines.append("        %in_range = arith.cmpi ult, %col, %cN : index")
        lines.append("        %xv0 = scf.if %in_range -> (f32) {")
        lines.append(f"          %xv = memref.load {arg_ssa[x_name]}[%col] : {x_memref}")
        lines.append("          scf.yield %xv : f32")
        lines.append("        } else {")
        lines.append("          scf.yield %neg_inf : f32")
        lines.append("        }")
        ...
        # Warp inclusive scan (shuffle up).
        lines.append("        %s1, %ok1 = gpu.shuffle up %xv0, %c1_i32, %c32_i32 : f32")
        lines.append(f"        %a1 = arith.maximumf %xv0, %s1{fm} : f32")
        lines.append("        %v1 = arith.select %ok1, %a1, %xv0 : f32")
        lines.append("        %s2, %ok2 = gpu.shuffle up %v1, %c2_i32, %c32_i32 : f32")
        lines.append(f"        %a2 = arith.maximumf %v1, %s2{fm} : f32")
        lines.append("        %v2 = arith.select %ok2, %a2, %v1 : f32")
        lines.append("        %s4, %ok4 = gpu.shuffle up %v2, %c4_i32, %c32_i32 : f32")
        lines.append(f"        %a4 = arith.maximumf %v2, %s4{fm} : f32")
        lines.append("        %v4 = arith.select %ok4, %a4, %v2 : f32")
        lines.append("        %s8, %ok8 = gpu.shuffle up %v4, %c8_i32, %c32_i32 : f32")
        lines.append(f"        %a8 = arith.maximumf %v4, %s8{fm} : f32")
        lines.append("        %v8 = arith.select %ok8, %a8, %v4 : f32")
        lines.append("        %s16, %ok16 = gpu.shuffle up %v8, %c16_i32, %c32_i32 : f32")
        lines.append(f"        %a16 = arith.maximumf %v8, %s16{fm} : f32")
        lines.append("        %v16 = arith.select %ok16, %a16, %v8 : f32")
        ...
        lines.append("          }")
        lines.append("          %ws1, %wok1 = gpu.shuffle up %wv0, %c1_i32, %c32_i32 : f32")
        lines.append(f"          %wa1 = arith.maximumf %wv0, %ws1{fm} : f32")
        lines.append("          %wv1 = arith.select %wok1, %wa1, %wv0 : f32")
        lines.append("          %ws2, %wok2 = gpu.shuffle up %wv1, %c2_i32, %c32_i32 : f32")
        lines.append(f"          %wa2 = arith.maximumf %wv1, %ws2{fm} : f32")
        lines.append("          %wv2 = arith.select %wok2, %wa2, %wv1 : f32")
        lines.append("          %ws4, %wok4 = gpu.shuffle up %wv2, %c4_i32, %c32_i32 : f32")
        lines.append(f"          %wa4 = arith.maximumf %wv2, %ws4{fm} : f32")
        lines.append("          %wv4 = arith.select %wok4, %wa4, %wv2 : f32")
        lines.append("          %ws8, %wok8 = gpu.shuffle up %wv4, %c8_i32, %c32_i32 : f32")
        lines.append(f"          %wa8 = arith.maximumf %wv4, %ws8{fm} : f32")
        lines.append("          %wv8 = arith.select %wok8, %wa8, %wv4 : f32")
        lines.append("          %ws16, %wok16 = gpu.shuffle up %wv8, %c16_i32, %c32_i32 : f32")
        lines.append(f"          %wa16 = arith.maximumf %wv8, %ws16{fm} : f32")
        lines.append("          %wv16 = arith.select %wok16, %wa16, %wv8 : f32")
        lines.append("          %wprev, %wok_prev = gpu.shuffle up %wv16, %c1_i32, %c32_i32 : f32")
        lines.append("          %woff = arith.select %wok_prev, %wprev, %neg_inf : f32")
        lines.append("          scf.if %lane_in {")
        lines.append("            %off_idx = arith.addi %cWarps, %lane : index")
        lines.append("            memref.store %woff, %sh[%off_idx] : memref<256xf32, 3>")
        lines.append("          }")
        ...
        lines.append("        %off_idx2 = arith.addi %cWarps, %warp : index")
        lines.append("        %woff2 = memref.load %sh[%off_idx2] : memref<256xf32, 3>")
        lines.append(f"        %block_scan = arith.maximumf %v16, %woff2{fm} : f32")
        lines.append(f"        %full_scan = arith.maximumf %block_scan, %carry0{fm} : f32")
        lines.append("        scf.if %in_range {")
        lines.append(f"          memref.store %full_scan, {arg_ssa[out_name2]}[%col] : {out_memref}")
        lines.append("        }")
        lines.append("        %is_last = arith.cmpi eq, %tid, %c255 : index")
        lines.append("        scf.if %is_last {")
        lines.append("          memref.store %full_scan, %sh[%cCarry] : memref<256xf32, 3>")
        ...
        lines.append("      %is_tid0 = arith.cmpi eq, %tid, %c0 : index")
        lines.append("      scf.if %is_tid0 {")
        lines.append("        memref.store %pos_inf, %sh[%cCarry] : memref<256xf32, 3>")
        lines.append("      }")
        lines.append("      gpu.barrier")
        ...
        # Warp inclusive scan (shuffle up).
        lines.append("        %s1, %ok1 = gpu.shuffle up %xv0, %c1_i32, %c32_i32 : f32")
        lines.append(f"        %a1 = arith.minimumf %xv0, %s1{fm} : f32")
        lines.append("        %v1 = arith.select %ok1, %a1, %xv0 : f32")
        lines.append("        %s2, %ok2 = gpu.shuffle up %v1, %c2_i32, %c32_i32 : f32")
        lines.append(f"        %a2 = arith.minimumf %v1, %s2{fm} : f32")
        lines.append("        %v2 = arith.select %ok2, %a2, %v1 : f32")
        lines.append("        %s4, %ok4 = gpu.shuffle up %v2, %c4_i32, %c32_i32 : f32")
        lines.append(f"        %a4 = arith.minimumf %v2, %s4{fm} : f32")
        lines.append("        %v4 = arith.select %ok4, %a4, %v2 : f32")
        lines.append("        %s8, %ok8 = gpu.shuffle up %v4, %c8_i32, %c32_i32 : f32")
        lines.append(f"        %a8 = arith.minimumf %v4, %s8{fm} : f32")
        lines.append("        %v8 = arith.select %ok8, %a8, %v4 : f32")
        lines.append("        %s16, %ok16 = gpu.shuffle up %v8, %c16_i32, %c32_i32 : f32")
        lines.append(f"        %a16 = arith.minimumf %v8, %s16{fm} : f32")
        lines.append("        %v16 = arith.select %ok16, %a16, %v8 : f32")
        ...
        lines.append("          }")
        lines.append("          %ws1, %wok1 = gpu.shuffle up %wv0, %c1_i32, %c32_i32 : f32")
        lines.append(f"          %wa1 = arith.minimumf %wv0, %ws1{fm} : f32")
        lines.append("          %wv1 = arith.select %wok1, %wa1, %wv0 : f32")
        lines.append("          %ws2, %wok2 = gpu.shuffle up %wv1, %c2_i32, %c32_i32 : f32")
        lines.append(f"          %wa2 = arith.minimumf %wv1, %ws2{fm} : f32")
        lines.append("          %wv2 = arith.select %wok2, %wa2, %wv1 : f32")
        lines.append("          %ws4, %wok4 = gpu.shuffle up %wv2, %c4_i32, %c32_i32 : f32")
        lines.append(f"          %wa4 = arith.minimumf %wv2, %ws4{fm} : f32")
        lines.append("          %wv4 = arith.select %wok4, %wa4, %wv2 : f32")
        lines.append("          %ws8, %wok8 = gpu.shuffle up %wv4, %c8_i32, %c32_i32 : f32")
        lines.append(f"          %wa8 = arith.minimumf %wv4, %ws8{fm} : f32")
        lines.append("          %wv8 = arith.select %wok8, %wa8, %wv4 : f32")
        lines.append("          %ws16, %wok16 = gpu.shuffle up %wv8, %c16_i32, %c32_i32 : f32")
        lines.append(f"          %wa16 = arith.minimumf %wv8, %ws16{fm} : f32")
        lines.append("          %wv16 = arith.select %wok16, %wa16, %wv8 : f32")
        lines.append("          %wprev, %wok_prev = gpu.shuffle up %wv16, %c1_i32, %c32_i32 : f32")
        lines.append("          %woff = arith.select %wok_prev, %wprev, %pos_inf : f32")
        lines.append("          scf.if %lane_in {")
        lines.append("            %off_idx = arith.addi %cWarps, %lane : index")
        lines.append("            memref.store %woff, %sh[%off_idx] : memref<256xf32, 3>")
        lines.append("          }")
        ...
        lines.append("        %off_idx2 = arith.addi %cWarps, %warp : index")
        lines.append("        %woff2 = memref.load %sh[%off_idx2] : memref<256xf32, 3>")
        lines.append(f"        %block_scan = arith.minimumf %v16, %woff2{fm} : f32")
        lines.append(f"        %full_scan = arith.minimumf %block_scan, %carry0{fm} : f32")
        lines.append("        scf.if %in_range {")
        lines.append(f"          memref.store %full_scan, {arg_ssa[out_name2]}[%col] : {out_memref}")
        lines.append("        }")
        lines.append("        %is_last = arith.cmpi eq, %tid, %c255 : index")
        ...
        m_lane = _fresh("m_lane")
        alpha_lane = _fresh("alpha_lane")
        p_lane = _fresh("p_lane")
        l_next_lane = _fresh("l_next_lane")
        lines.append(f"          {m_lane}, {alpha_lane}, {p_lane}, {l_next_lane} = scf.if %is0 -> (f32, f32, f32, f32) {{")
        m_new0 = _fresh("m_new0")
        delta0 = _fresh("delta0")
        alpha0 = _fresh("alpha0")
        l_scaled0 = _fresh("l_scaled0")
        s_delta0 = _fresh("s_delta0")
        p0 = _fresh("p0")
        l_next0 = _fresh("l_next0")
        lines.append(f"            {m_new0} = arith.maximumf %m, {score_ssa}{fm} : f32")
        lines.append(f"            {delta0} = arith.subf %m, {m_new0}{fm} : f32")
        lines.append(f"            {alpha0} = math.exp {delta0}{fm} : f32")
        lines.append(f"            {l_scaled0} = arith.mulf %l, {alpha0}{fm} : f32")
        lines.append(f"            {s_delta0} = arith.subf {score_ssa}, {m_new0}{fm} : f32")
        lines.append(f"            {p0} = math.exp {s_delta0}{fm} : f32")
        lines.append(f"            {l_next0} = arith.addf {l_scaled0}, {p0}{fm} : f32")
        lines.append(f"            scf.yield {m_new0}, {alpha0}, {p0}, {l_next0} : f32, f32, f32, f32")
        lines.append("          } else {")
        lines.append("            scf.yield %m, %c0f, %c0f, %l : f32, f32, f32, f32")
        lines.append("          }")
        ...
        m_ok = _fresh("m_ok")
        m_new = _fresh("m_new")
        lines.append(f"          {m_sh}, {m_ok} = gpu.shuffle idx {m_lane}, %c0_i32, %c32_i32 : f32")
        lines.append(f"          {m_new} = arith.select {m_ok}, {m_sh}, %m : f32")

        alpha_sh = _fresh("alpha_sh")
        alpha_ok = _fresh("alpha_ok")
        alpha = _fresh("alpha")
        lines.append(f"          {alpha_sh}, {alpha_ok} = gpu.shuffle idx {alpha_lane}, %c0_i32, %c32_i32 : f32")
        lines.append(f"          {alpha} = arith.select {alpha_ok}, {alpha_sh}, %c0f : f32")

        p_sh = _fresh("p_sh")
        p_ok = _fresh("p_ok")
        p = _fresh("p")
        lines.append(f"          {p_sh}, {p_ok} = gpu.shuffle idx {p_lane}, %c0_i32, %c32_i32 : f32")
        lines.append(f"          {p} = arith.select {p_ok}, {p_sh}, %c0f : f32")

        l_next_sh = _fresh("l_next_sh")
        l_next_ok = _fresh("l_next_ok")
        l_next = _fresh("l_next")
        lines.append(f"          {l_next_sh}, {l_next_ok} = gpu.shuffle idx {l_next_lane}, %c0_i32, %c32_i32 : f32")
        lines.append(f"          {l_next} = arith.select {l_next_ok}, {l_next_sh}, %l : f32")
