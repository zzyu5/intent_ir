...
def relu2d(inp: torch.Tensor) -> torch.Tensor:
    if inp.dtype != torch.float32:
        raise TypeError("relu2d expects float32 tensor")
    if inp.ndim != 2:
        raise ValueError(f"relu2d expects rank-2 tensor, got {inp.ndim}")
    m, n = inp.shape
