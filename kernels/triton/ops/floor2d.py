...
def floor2d(inp: torch.Tensor) -> torch.Tensor:
    if inp.dtype != torch.float32:
        raise TypeError("floor2d expects float32 tensor")
    if inp.ndim != 2:
        raise ValueError(f"floor2d expects rank-2 tensor, got {inp.ndim}")
    m, n = inp.shape
...
__all__ = ["floor2d_kernel", "floor2d"]
