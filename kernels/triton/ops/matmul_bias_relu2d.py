...
def matmul_bias_relu2d(A: torch.Tensor, B: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    if A.dtype != torch.float32 or B.dtype != torch.float32 or bias.dtype != torch.float32:
        raise TypeError("matmul_bias_relu2d expects float32 tensors")
    if A.ndim != 2 or B.ndim != 2 or bias.ndim != 1:
        raise ValueError("matmul_bias_relu2d expects A,B rank-2 and bias rank-1")
    m, k = A.shape
    k2, n = B.shape
    if k2 != k:
...
__all__ = ["matmul_bias_relu2d_kernel", "matmul_bias_relu2d"]
