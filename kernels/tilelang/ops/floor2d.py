def make_floor2d_prim_func(*, n: int = 16, threads: int = 128):
    """
    Elementwise floor: out = floor(inp) for 2D float32 tensors.
    ...
    def main(
        ...
        with T.Kernel(M, threads=threads) as (pid_m,):
            ...
            for r in T.serial(N):
                out_row[r] = T.floor(row[r])
            T.copy(out_row, out[pid_m, 0])

    return main


__all__ = ["make_floor2d_prim_func"]
