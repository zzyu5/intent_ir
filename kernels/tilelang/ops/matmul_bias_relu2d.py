def make_matmul_bias_relu2d_prim_func(
    *,
    block_m: int = 64,
    block_n: int = 64,
    block_k: int = 32,
    ...
    """
    GEMM(+bias+ReLU) micro-kernel in float32.

    Used as a more complex coverage kernel (matmul + broadcast + relu).
    """
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(N, block_n), T.ceildiv(M, block_m), threads=threads) as (bx, by):
            ...
            for i, j in T.Parallel(block_m, block_n):
                C_local[i, j] = T.max(C_local[i, j] + bias_local[j], zero)
            T.copy(C_local, C[by * block_m, bx * block_n])

    return main


__all__ = ["make_matmul_bias_relu2d_prim_func"]
