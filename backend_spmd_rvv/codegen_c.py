...
use `backend_spmd_rvv.lower_c.lower_intent_to_c_with_files`. This module remains
...
from .tiling_search import TileChoice, SPMDProfile
...
def _find_first_matmul(intent: IntentFunction) -> Op:
    for op in intent.ops:
        if op.op == "matmul":
            return op
    raise IntentIRValidationError("codegen_c currently supports only matmul intents")
...
def generate_c(intent: IntentFunction, tile: TileChoice, profile: SPMDProfile, *, func_name: Optional[str] = None) -> str:
    ...
    lines.append("")
    lines.append("/* Auto-generated by backend_spmd_rvv.codegen_c (Task6 MVP) */")
