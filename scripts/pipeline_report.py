"""
End-to-end status script for Tasks 1-5 on the three real Triton ops.

What it does for each op (any_kernel_dim, group_norm_kernel, _attn_fwd):
1) Extracts the Triton kernel source and calls the LLM to produce Intent-IR JSON.
2) Parses to IntentFunction and prints MLIR-like pretty form.
3) Launches the real Triton kernel with a minimal config to trigger TTIR dump
   (uses TRITON_KERNEL_DUMP / TRITON_DUMP_DIR), then scans TTIR facts and
   evaluates the Task4 contract.

Run:
  TRITON_KERNEL_DUMP=1 TRITON_DUMP_DIR=/tmp/triton_dump python scripts/pipeline_report.py
You can set SKIP_LLM=1 to skip the LLM calls and only run kernels/TTIR.
"""
...
from triton_frontend.facts import extract_facts
from triton_frontend.contract import evaluate_contract
from tritonops.ttir_launch import (
...
def main() -> None:
    _run_one("ANY (reduce_any)", "tritonops.any", "any_kernel_dim", launch_any_kernel_dim)
    _run_one("GroupNorm", "tritonops.groupnorm", "group_norm_kernel", launch_group_norm_kernel)
    _run_one("Attention _attn_fwd", "tritonops.attention", "_attn_fwd", launch_attention_kernel)
