...
- 统一入口：
  - `intent_ir.llm.chat_completion`（底层 API 封装）
  - `frontends.triton.llm_intent.extract_intent_json`（从 Triton 源码提取 Intent-IR v1.1 候选 JSON）
- 示例（使用本地 providers 文件）：
  ```bash
  python -c "from frontends.triton.llm_intent import extract_intent_json; print(extract_intent_json(open('kernel.py').read()))"
  ```
