...
def _kernel_fused_elementwise(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ...
    Supports:
      - unary/binary float ops: add/sub/mul/div/pow/max/min/relu/abs/exp/log/sin/cos/erf/floor/sqrt/rsqrt
    ...
    for op in intent.ops or []:
        ...
        if opname == "const":
            ...
            if len(op.inputs) != 1:
                raise CudaLoweringError("sin expects 1 input")
            x = val(op.inputs[0])
            emit_assign(f"sinf({x})")
        elif opname == "acos":
            if len(op.inputs) != 1:
            ...
            if len(op.inputs) != 1:
                raise CudaLoweringError("sqrt expects 1 input")
            x = val(op.inputs[0])
            emit_assign(f"sqrtf({x})")
        elif opname == "ceil":
            if len(op.inputs) != 1:
            ...
            x = val(op.inputs[0])
            if str(out_dt) == "f16":
                in_dt = _dtype_of(op.inputs[0])
                if str(in_dt) == "f16":
                    emit_assign(x)
                else:
                    emit_assign(f"__float2half((float)({x}))")
            else:
                emit_assign(f"({cty})({x})")
        else:
    ...
    needs_fp16 = any(str(intent.tensors.get(n).dtype) == "f16" for n in tensor_args if n in intent.tensors)
...
def _kernel_rms_norm_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    produced = {o.output for o in (intent.ops or [])}
    outs = set(intent.outputs or [])
    inputs = [n for n in intent.tensors.keys() if n not in produced and n not in outs]
    ...
    for n in inputs:
        t = intent.tensors[n]
        if str(t.dtype) == "f32" and len(t.shape or []) == 2:
            x_name = n
            break
    if not x_name:
        raise CudaLoweringError("rms_norm lowering cannot find rank-2 f32 input tensor")

    x_shape = _shape_values(intent, x_name)
    if len(x_shape) != 2:
        raise CudaLoweringError("rms_norm expects rank-2 input")
    M_dim, N_dim = x_shape
    M = _resolve_dim_int(M_dim, bindings, name="M")
    N = _resolve_dim_int(N_dim, bindings, name="N")
    ...
    for n in inputs:
        if n == x_name:
            continue
        t = intent.tensors[n]
        if str(t.dtype) != "f32" or len(t.shape or []) != 1:
            continue
        w_n = _shape_values(intent, n)
        if len(w_n) == 1 and _resolve_dim_int(w_n[0], bindings, name="N_weight") == N:
            w_name = n
            break
    if not w_name:
        raise CudaLoweringError("rms_norm lowering cannot find weight vector")

    out_names = [str(n) for n in (intent.outputs or [])]
    if len(out_names) != 2:
        raise CudaLoweringError("rms_norm lowering expects exactly 2 outputs: [out, inv_rms]")

    y_name: str | None = None
    inv_name: str | None = None
    for n in out_names:
        shp = _shape_values(intent, n)
        if len(shp) == 2 and _resolve_dim_int(shp[0], bindings, name=f"{n}_M") == M and _resolve_dim_int(shp[1], bindings, name=f"{n}_N") == N:
            y_name = n
            continue
        if len(shp) == 1 and _resolve_dim_int(shp[0], bindings, name=f"{n}_M") == M:
            inv_name = n
            continue
        if len(shp) == 2:
            m_ok = _resolve_dim_int(shp[0], bindings, name=f"{n}_M") == M
            n_ok = _resolve_dim_int(shp[1], bindings, name=f"{n}_N") == 1
            if m_ok and n_ok:
                inv_name = n
                continue
    if not y_name or not inv_name:
        raise CudaLoweringError("rms_norm output shape mismatch")

    inv_shape = _shape_values(intent, inv_name)
    inv_rank2 = len(inv_shape) == 2
    inv_stride = 1 if not inv_rank2 else _resolve_dim_int(inv_shape[1], bindings, name="INV_STRIDE")

    eps = None
    for op in intent.ops or []:
        if str(op.op) != "const":
            continue
        if str(op.output) == "eps":
            try:
                eps = float(op.attrs.get("value"))
            except Exception:
                eps = None
    ...
    inv_store = f"{inv_name}[row] = inv_rms;"
    if inv_rank2:
        inv_store = f"{inv_name}[(size_t)row * (size_t){inv_stride}] = inv_rms;"
...
extern "C" __global__ void {intent.name}(
  ...
  const float* x_row = {x_name} + (size_t)row * (size_t)N;
  float sq_sum = 0.0f;
  for (int col = 0; col < N; ++col) {{
    const float v = x_row[(size_t)col];
    sq_sum += v * v;
  }}
  const float mean = sq_sum / (float)N;
  const float inv_rms = rsqrtf(mean + eps);
  {inv_store}

  float* y_row = {y_name} + (size_t)row * (size_t)N;
  for (int col = 0; col < N; ++col) {{
    y_row[(size_t)col] = x_row[(size_t)col] * inv_rms * {w_name}[(size_t)col];
  }}
}}
""".lstrip()

    tensor_args = [x_name, w_name, y_name, inv_name]
    scalar_args: Dict[str, str] = {"eps": "f32"}
    arg_names = [x_name, w_name, y_name, inv_name]
    for dim_name in ["M", "N"]:
        if _is_scalar_tensor(intent, dim_name, dtype="i32"):
            ...
            arg_names.append(dim_name)
    arg_names.append("eps")

    io_spec = _io_spec_from_args(intent, tensor_args=tensor_args, scalar_args=scalar_args, arg_names=arg_names)
    launch = CudaLaunch(grid=(M, 1, 1), block=(32, 1, 1), shared_mem=0)
    out_bindings: Dict[str, Any] = dict(bindings)
    out_bindings.setdefault("M", int(M))
    out_bindings.setdefault("N", int(N))
    out_bindings.setdefault("eps", float(eps))
    return CudaLoweredKernel(
...
def _kernel_reduce_min_2d_all_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "reduce_min":
        raise CudaLoweringError("reduce_min(all) lowering expects a single reduce_min op")
    op = intent.ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("reduce_min(all) expects 1 input")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    ...
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("reduce_min(all) lowering currently supports rank-2 input only")
    if len(out_shape) != 0:
        raise CudaLoweringError("reduce_min(all) lowering expects scalar output")
    if str(intent.tensors[inp_name].dtype) != "f32" or str(intent.tensors[out_name].dtype) != "f32":
        raise CudaLoweringError("reduce_min(all) lowering currently supports f32 tensors")

    M = _as_int(bindings.get("M"), name="M")
    N = _as_int(bindings.get("N"), name="N")
    total = max(0, int(M) * int(N))
    sched = intent.schedule or ScheduleSketch()
    block_x = _resolve_schedule_int(sched.tile_n, bindings, default=256)
    if block_x <= 0:
        block_x = 256
...
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(const float* __restrict__ {inp_name}, float* __restrict__ {out_name}, {m_param}, {n_param}) {{
  ...
  const int total = M * N;
  float acc = INFINITY;
  for (int idx = (int)threadIdx.x; idx < total; idx += (int)blockDim.x) acc = fminf(acc, intentir_ldg_f32({inp_name} + (size_t)idx));
  const float mn = intentir_cuda::block_allreduce_min<BLOCK_THREADS>(acc, &red);
  if ((int)threadIdx.x == 0) {out_name}[0] = (total > 0) ? mn : INFINITY;
}}
""".lstrip()
...
def _kernel_row_mean_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ...
    if len(ops) != 3:
        raise CudaLoweringError("row_mean lowering expects 3 ops: const, reduce_sum, div")
    const_ops = [o for o in ops if o.op == "const"]
    reduce_ops = [o for o in ops if o.op == "reduce_sum"]
    div_ops = [o for o in ops if o.op == "div"]
    if len(const_ops) != 1 or len(reduce_ops) != 1 or len(div_ops) != 1:
        raise CudaLoweringError("row_mean lowering expects const+reduce_sum+div pattern")
    red = reduce_ops[0]
    div = div_ops[0]
    if len(red.inputs) != 1 or len(div.inputs) != 2:
        raise CudaLoweringError("row_mean pattern has invalid op arity")
    inp_name = str(red.inputs[0])
    out_name = str(div.output)
    red_out = str(red.output)
    const_out = str(const_ops[0].output)
    if sorted([str(x) for x in div.inputs]) != sorted([red_out, const_out]):
        raise CudaLoweringError("row_mean div must consume reduce_sum output and const scalar")
    dims = (red.attrs or {}).get("dims")
    axis = (red.attrs or {}).get("axis")
    if dims not in ([1], (1,)) and axis not in ([1], 1, "1"):
        raise CudaLoweringError("row_mean currently supports axis=1 reductions only")
    if str(intent.tensors[inp_name].dtype) != "f32" or str(intent.tensors[out_name].dtype) != "f32":
        raise CudaLoweringError("row_mean lowering currently supports f32 input/output")
...
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(const float* __restrict__ {inp_name}, float* __restrict__ {out_name}, {m_param}, {n_param}) {{
  ...
  for (int n = (int)threadIdx.x; n < N; n += (int)blockDim.x) acc += intentir_ldg_f32(row + (size_t)n);
  const float sum = intentir_cuda::block_allreduce_sum<BLOCK_THREADS>(acc, &red);
  if ((int)threadIdx.x == 0) {out_name}[m] = sum / (float)N;
}}
""".lstrip()
...
def _kernel_mse_loss_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "mse_loss":
        raise CudaLoweringError("mse_loss lowering expects a single mse_loss op")
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("mse_loss expects two inputs (inp, target)")
    inp_name, target_name = (str(x) for x in op.inputs)
    out_name = str(op.output)
    if str(intent.tensors[inp_name].dtype) != "f32" or str(intent.tensors[target_name].dtype) != "f32" or str(intent.tensors[out_name].dtype) != "f32":
        raise CudaLoweringError("mse_loss lowering currently supports f32 tensors only")

    inp_shape = _shape_values(intent, inp_name)
    target_shape = _shape_values(intent, target_name)
    out_shape = _shape_values(intent, out_name)
    if len(inp_shape) != 2 or len(target_shape) != 2:
        raise CudaLoweringError("mse_loss lowering expects rank-2 inputs")
    if inp_shape != target_shape:
        raise CudaLoweringError("mse_loss input/target shape mismatch")
    if len(out_shape) != 0:
        raise CudaLoweringError("mse_loss lowering currently expects scalar output")

    M = _resolve_dim_int(inp_shape[0], bindings, name="M")
    N = _resolve_dim_int(inp_shape[1], bindings, name="N")
    reduction = (op.attrs or {}).get("reduction", 1)
    if reduction in {"none", 0}:
        raise CudaLoweringError("mse_loss reduction=none is not supported in this lowering")
    use_mean = reduction in {"mean", 1}

    cuda_src = f"""
#include <stdint.h>
extern "C" __global__ void {intent.name}(
    const float* __restrict__ {inp_name},
    const float* __restrict__ {target_name},
    float* __restrict__ {out_name},
    int M, int N) {{
  ...
  for (int i = 0; i < total; ++i) {{
    const float d = {inp_name}[i] - {target_name}[i];
    acc += (double)(d * d);
  }}
  const float v = {("(float)(acc / (double)total)" if use_mean else "(float)acc")};
  {out_name}[0] = v;
}}
""".lstrip()
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, target_name, out_name],
        scalar_args={"M": "i32", "N": "i32"},
        arg_names=[inp_name, target_name, out_name, "M", "N"],
    )
...
def _kernel_max_pool2d_with_indices_nchw_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if not ops or any(str(o.op) != "max_pool2d_with_indices" for o in ops):
        raise CudaLoweringError("max_pool2d_with_indices lowering expects only max_pool2d_with_indices ops")

    inp_name = str(ops[0].inputs[0]) if ops[0].inputs else ""
    if not inp_name:
        raise CudaLoweringError("max_pool2d_with_indices expects one input tensor")
    if str(intent.tensors[inp_name].dtype) != "f32":
        raise CudaLoweringError("max_pool2d_with_indices lowering currently supports f32 input only")

    values_out_name: str | None = None
    indices_out_name: str | None = None
    attrs = dict(ops[0].attrs or {})
    attrs_norm = {str(k): v for k, v in attrs.items() if str(k) != "select"}
    for op in ops:
        if len(op.inputs) != 1 or str(op.inputs[0]) != inp_name:
            raise CudaLoweringError("max_pool2d_with_indices ops must share the same single input")
        op_attrs = dict(op.attrs or {})
        select = str(op_attrs.get("select", "values")).strip().lower()
        out_name = str(op.output)
        if select == "indices":
            indices_out_name = out_name
        else:
            values_out_name = out_name
        op_norm = {str(k): v for k, v in op_attrs.items() if str(k) != "select"}
        if op_norm != attrs_norm:
            raise CudaLoweringError("max_pool2d_with_indices currently requires identical attrs for value/index outputs")

    if values_out_name is None and indices_out_name is None:
        raise CudaLoweringError("max_pool2d_with_indices must produce at least one output")
    if values_out_name is not None and str(intent.tensors[values_out_name].dtype) != "f32":
        raise CudaLoweringError("max_pool2d_with_indices values output must be f32")
    if indices_out_name is not None and str(intent.tensors[indices_out_name].dtype) != "i64":
        raise CudaLoweringError("max_pool2d_with_indices indices output must be i64")

    in_shape = _shape_values(intent, inp_name)
    if len(in_shape) != 4:
        raise CudaLoweringError("max_pool2d_with_indices expects input shape [N,C,H,W]")
    ...
    def _pair(val: Any, default: int) -> tuple[int, int]:
        ...
        if isinstance(val, (list, tuple)):
            if len(val) != 2:
                raise CudaLoweringError("max_pool2d_with_indices expects pair attrs")
            return (
                _resolve_attr_int(val[0], bindings, name="max_pool2d_with_indices.pair[0]"),
                _resolve_attr_int(val[1], bindings, name="max_pool2d_with_indices.pair[1]"),
            )
        v = _resolve_attr_int(val, bindings, name="max_pool2d_with_indices.pair")
        return (v, v)
    ...
    if ceil_mode:
        raise CudaLoweringError("max_pool2d_with_indices ceil_mode=true is not supported")
    if dh != 1 or dw != 1:
        raise CudaLoweringError("max_pool2d_with_indices dilation != 1 is not supported")
    if sh <= 0 or sw <= 0:
        raise CudaLoweringError("max_pool2d_with_indices stride must be positive")
    OH = ((H + 2 * ph - kh) // sh) + 1
    OW = ((W + 2 * pw - kw) // sw) + 1
    if OH <= 0 or OW <= 0:
        raise CudaLoweringError("max_pool2d_with_indices invalid output shape")

    if values_out_name is not None:
        vshape = _shape_values(intent, values_out_name)
        if [N, C, OH, OW] != [_resolve_dim_int(vshape[0], bindings, name="values.N"), _resolve_dim_int(vshape[1], bindings, name="values.C"), _resolve_dim_int(vshape[2], bindings, name="values.OH"), _resolve_dim_int(vshape[3], bindings, name="values.OW")]:
            raise CudaLoweringError("max_pool2d_with_indices values output shape mismatch")
    if indices_out_name is not None:
        ishape = _shape_values(intent, indices_out_name)
        if [N, C, OH, OW] != [_resolve_dim_int(ishape[0], bindings, name="indices.N"), _resolve_dim_int(ishape[1], bindings, name="indices.C"), _resolve_dim_int(ishape[2], bindings, name="indices.OH"), _resolve_dim_int(ishape[3], bindings, name="indices.OW")]:
            raise CudaLoweringError("max_pool2d_with_indices indices output shape mismatch")
    ...
    values_param = f"float* __restrict__ {values_out_name}" if values_out_name is not None else "float* __restrict__ out_values_unused"
    indices_param = f"int64_t* __restrict__ {indices_out_name}" if indices_out_name is not None else "int64_t* __restrict__ out_indices_unused"
    values_store = f"{values_out_name}[tid] = best;" if values_out_name is not None else "(void)best;"
    indices_store = f"{indices_out_name}[tid] = (int64_t)best_idx;" if indices_out_name is not None else "(void)best_idx;"
...
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(
  ...
  for (int kh_i = 0; kh_i < {kh}; ++kh_i) {{
    const int ih = h_start + kh_i;
    if ((unsigned)ih >= (unsigned)H) continue;
    for (int kw_i = 0; kw_i < {kw}; ++kw_i) {{
      const int iw = w_start + kw_i;
      if ((unsigned)iw >= (unsigned)W) continue;
      const int64_t in_pos = (((int64_t)n * (int64_t)C + (int64_t)c) * (int64_t)H + (int64_t)ih) * (int64_t)W + (int64_t)iw;
      const float v = {inp_name}[in_pos];
...
""".lstrip()
    ...
    tensor_args = [inp_name]
    arg_names = [inp_name]
    if values_out_name is not None:
        tensor_args.append(values_out_name)
        arg_names.append(values_out_name)
    else:
        arg_names.append("out_values_unused")
    if indices_out_name is not None:
        tensor_args.append(indices_out_name)
        arg_names.append(indices_out_name)
    else:
        arg_names.append("out_indices_unused")
    scalar_args = {"N": "i32", "C": "i32", "H": "i32", "W": "i32", "OH": "i32", "OW": "i32"}
    arg_names.extend(["N", "C", "H", "W", "OH", "OW"])
    io_spec = _io_spec_from_args(intent, tensor_args=tensor_args, scalar_args=scalar_args, arg_names=arg_names)
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    ...
    out_names = []
    if values_out_name is not None:
        out_names.append(values_out_name)
    if indices_out_name is not None:
        out_names.append(indices_out_name)
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=out_names, bindings=lowered_bindings)


def _kernel_reduce_min_with_aux_pattern_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    reduce_ops = [o for o in (intent.ops or []) if str(o.op) == "reduce_min"]
    if not reduce_ops:
        raise CudaLoweringError("reduce_min aux-pattern lowering expects a reduce_min op")
    red = reduce_ops[0]
    if len(red.inputs) != 1:
        raise CudaLoweringError("reduce_min aux-pattern expects a single reduce_min input")
    inp_name = str(red.inputs[0])
    out_name = str(red.output)
    if inp_name not in intent.tensors or out_name not in intent.tensors:
        raise CudaLoweringError("reduce_min aux-pattern requires input/output tensors in intent")
    synthetic = IntentFunction.from_json_dict(
        {
            "name": str(intent.name),
            "tensors": {
                str(inp_name): {"dtype": "f32", "shape": _shape_values(intent, inp_name), "layout": "row_major"},
                str(out_name): {"dtype": "f32", "shape": _shape_values(intent, out_name), "layout": "row_major"},
            },
            "ops": [
                {
                    "op": "reduce_min",
            ...
            "parallel_axes": [str(x) for x in (intent.parallel_axes or [])],
            "schedule": _schedule_to_json_dict(intent.schedule),
        }
    )
    return _kernel_reduce_min_2d_axis1_f32(synthetic, bindings)


def _kernel_mm_cast_chain_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 2 or str(ops[0].op) != "matmul" or str(ops[1].op) != "cast":
        raise CudaLoweringError("mm cast-chain lowering expects matmul->cast")
    matmul, cast = ops
    if len(matmul.inputs) != 2 or len(cast.inputs) != 1:
        raise CudaLoweringError("mm cast-chain has invalid op arity")
    if str(cast.inputs[0]) != str(matmul.output):
        raise CudaLoweringError("mm cast-chain expects cast to consume matmul output")
    a_name = str(matmul.inputs[0])
    b_name = str(matmul.inputs[1])
    out_name = str(cast.output)
    if a_name not in intent.tensors or b_name not in intent.tensors or out_name not in intent.tensors:
        raise CudaLoweringError("mm cast-chain tensors missing from intent")
    synthetic = IntentFunction.from_json_dict(
        {
            "name": str(intent.name),
            "tensors": {
                str(a_name): {"dtype": "f32", "shape": _shape_values(intent, a_name), "layout": "row_major"},
                str(b_name): {"dtype": "f32", "shape": _shape_values(intent, b_name), "layout": "row_major"},
                str(out_name): {"dtype": "f32", "shape": _shape_values(intent, out_name), "layout": "row_major"},
            },
            "ops": [
                {
                    "op": "matmul",
                    "inputs": [str(a_name), str(b_name)],
                    "output": str(out_name),
                    "attrs": dict(matmul.attrs or {}),
                }
            ],
            "outputs": [str(out_name)],
            "parallel_axes": [str(x) for x in (intent.parallel_axes or [])],
    ...
    return _kernel_matmul_f32(synthetic, bindings)


def _kernel_minimum_cast_chain_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 4 or [str(o.op) for o in ops] != ["cast", "cast", "min", "cast"]:
        raise CudaLoweringError("minimum cast-chain lowering expects cast, cast, min, cast")
    c0, c1, min_op, c2 = ops
    x_name = str(c0.inputs[0]) if c0.inputs else ""
    y_name = str(c1.inputs[0]) if c1.inputs else ""
    out_name = str(c2.output)
    if not x_name or not y_name or out_name not in intent.tensors:
        raise CudaLoweringError("minimum cast-chain tensors missing")
    if len(min_op.inputs) != 2:
        raise CudaLoweringError("minimum cast-chain min op expects 2 inputs")
    if sorted([str(x) for x in min_op.inputs]) != sorted([str(c0.output), str(c1.output)]):
        raise CudaLoweringError("minimum cast-chain min must consume cast outputs")
    if str(c2.inputs[0]) != str(min_op.output):
        raise CudaLoweringError("minimum cast-chain final cast must consume min output")

    synthetic = IntentFunction.from_json_dict(
        {
            "name": str(intent.name),
            "tensors": {
                str(x_name): {"dtype": "f32", "shape": _shape_values(intent, x_name), "layout": "row_major"},
                str(y_name): {"dtype": "f32", "shape": _shape_values(intent, y_name), "layout": "row_major"},
                str(out_name): {"dtype": "f32", "shape": _shape_values(intent, out_name), "layout": "row_major"},
            },
            "ops": [
                {
                    "op": "min",
                    "inputs": [str(x_name), str(y_name)],
                    "output": str(out_name),
                }
            ],
            "outputs": [str(out_name)],
    ...
    return _kernel_fused_elementwise(synthetic, bindings)
...
def _kernel_isin_1d_i32_to_i1(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    """
    Pattern:
      broadcast_in_dim(in0->[M,K], [0]),
      broadcast_in_dim(in1->[M,K], [1]),
      ne, not, reduce_any(dims=[1]) -> out[M]
    """
    ops = list(intent.ops or [])
    if [o.op for o in ops] != ["broadcast_in_dim", "broadcast_in_dim", "ne", "not", "reduce_any"]:
        raise CudaLoweringError("isin lowering expects canonical broadcast/broadcast/ne/not/reduce_any chain")
    b0, b1, ne0, not0, red0 = ops
    in0_name = str(b0.inputs[0]) if b0.inputs else ""
    in1_name = str(b1.inputs[0]) if b1.inputs else ""
    if not in0_name or not in1_name:
        raise CudaLoweringError("isin lowering missing input tensors")
    if str(intent.tensors.get(in0_name).dtype) != "i32" or str(intent.tensors.get(in1_name).dtype) != "i32":
        raise CudaLoweringError("isin lowering currently supports i32 inputs only")
    if len(ne0.inputs) != 2 or str(not0.inputs[0]) != str(ne0.output) or str(red0.inputs[0]) != str(not0.output):
        raise CudaLoweringError("isin lowering op dependency mismatch")
    out_name = str(red0.output)
    if intent.outputs:
        out_name = str(intent.outputs[0])
    in0_shape = _shape_values(intent, in0_name)
    in1_shape = _shape_values(intent, in1_name)
    out_shape = _shape_values(intent, out_name)
    if len(in0_shape) != 1 or len(in1_shape) != 1 or len(out_shape) != 1:
        raise CudaLoweringError("isin lowering expects in0/in1/out rank-1")
    M = _resolve_dim_int(in0_shape[0], bindings, name="M")
    K = _resolve_dim_int(in1_shape[0], bindings, name="K")
    if _resolve_dim_int(out_shape[0], bindings, name="M_out") != M:
        raise CudaLoweringError("isin output shape mismatch")
    dims = (red0.attrs or {}).get("dims")
    axis = (red0.attrs or {}).get("axis")
    if dims not in ([1], (1,)) and axis not in ([1], 1, "1"):
        raise CudaLoweringError("isin reduction expects dims=[1]")
...
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(
    const int* __restrict__ {in0_name},
    const int* __restrict__ {in1_name},
  ...
  constexpr int BLOCK_THREADS = {block_x};
  __shared__ intentir_cuda::BlockAllreduceI32<BLOCK_THREADS> red;
  const int v = {in0_name}[m];
  int any = 0;
  for (int k = (int)threadIdx.x; k < K; k += (int)blockDim.x) {{
    any |= (v == {in1_name}[k]) ? 1 : 0;
  }}
  const int reduced = intentir_cuda::block_allreduce_max<BLOCK_THREADS>(any, &red);
  if ((int)threadIdx.x == 0) {out_name}[m] = (reduced != 0);
}}
""".lstrip()

    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[in0_name, in1_name, out_name],
        scalar_args={"M": "i32", "K": "i32"},
        arg_names=[in0_name, in1_name, out_name, "M", "K"],
    )
    launch = CudaLaunch(grid=(M, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"M": int(M), "K": int(K)})
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=[out_name], bindings=lowered_bindings)
...
def _kernel_scatter_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "scatter":
        raise CudaLoweringError("scatter lowering expects a single scatter op")
    op = intent.ops[0]
    if len(op.inputs) != 3:
        raise CudaLoweringError("scatter expects inputs [inp, index, src]")
    inp_name, index_name, src_name = (str(x) for x in op.inputs)
    out_name = str(op.output)

    axis = int((op.attrs or {}).get("dim", (op.attrs or {}).get("axis", 0)))
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("scatter lowering currently supports dim=1 only")

    in_shape = _shape_values(intent, inp_name)
    idx_shape = _shape_values(intent, index_name)
    src_shape = _shape_values(intent, src_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(idx_shape) != 2 or len(src_shape) != 2 or len(out_shape) != 2:
        raise CudaLoweringError("scatter lowering currently supports rank-2 tensors")

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if _resolve_dim_int(idx_shape[0], bindings, name="idx_M") != M or _resolve_dim_int(idx_shape[1], bindings, name="idx_N") != N:
        raise CudaLoweringError("scatter index shape mismatch")
    if _resolve_dim_int(src_shape[0], bindings, name="src_M") != M or _resolve_dim_int(src_shape[1], bindings, name="src_N") != N:
        raise CudaLoweringError("scatter src shape mismatch")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != N:
        raise CudaLoweringError("scatter out shape mismatch")

    cuda_src = f"""
#include <stdint.h>
extern "C" __global__ void {intent.name}(
    const float* __restrict__ {inp_name},
    const int* __restrict__ {index_name},
    const float* __restrict__ {src_name},
    float* __restrict__ {out_name},
    int M, int N) {{
  const int row = (int)blockIdx.x;
  if (row >= M) return;
  ...
  for (int col = 0; col < N; ++col) {{
    {out_name}[base + (size_t)col] = {inp_name}[base + (size_t)col];
  }}
  for (int col = 0; col < N; ++col) {{
    const int dst = {index_name}[base + (size_t)col];
    if ((unsigned)dst < (unsigned)N) {{
      {out_name}[base + (size_t)dst] = {src_name}[base + (size_t)col];
    }}
  }}
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, index_name, src_name, out_name],
        scalar_args={"M": "i32", "N": "i32"},
        arg_names=[inp_name, index_name, src_name, out_name, "M", "N"],
    )
    launch = CudaLaunch(grid=(M, 1, 1), block=(32, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
...
def _kernel_select_scatter_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "select_scatter":
        raise CudaLoweringError("select_scatter lowering expects a single select_scatter op")
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("select_scatter expects inputs [inp, src]")
    inp_name, src_name = (str(x) for x in op.inputs)
    out_name = str(op.output)
    ...
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("select_scatter lowering currently supports dim=1 only")
    index = int((op.attrs or {}).get("index", 0))

    in_shape = _shape_values(intent, inp_name)
    src_shape = _shape_values(intent, src_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(out_shape) != 2 or len(src_shape) != 1:
        raise CudaLoweringError("select_scatter lowering expects inp/out rank-2 and src rank-1")
    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if _resolve_dim_int(src_shape[0], bindings, name="src_M") != M:
        raise CudaLoweringError("select_scatter src shape mismatch")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != N:
        raise CudaLoweringError("select_scatter out shape mismatch")
    if index < 0:
        index += N
    if index < 0 or index >= N:
        raise CudaLoweringError("select_scatter index out of range")
...
extern "C" __global__ void {intent.name}(
  ...
  {out_name}[base + (size_t)INDEX_COL] = {src_name}[(size_t)row];
}}
""".lstrip()

    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, src_name, out_name],
        scalar_args={"M": "i32", "N": "i32", "INDEX_COL": "i32"},
        arg_names=[inp_name, src_name, out_name, "M", "N", "INDEX_COL"],
    )
    launch = CudaLaunch(grid=(M, 1, 1), block=(32, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.setdefault("M", int(M))
    lowered_bindings.setdefault("N", int(N))
    lowered_bindings["INDEX_COL"] = int(index)
    return CudaLoweredKernel(
        kernel_name=intent.name,
        cuda_src=cuda_src,
        io_spec=io_spec,
...
def _kernel_slice_scatter_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "slice_scatter":
        raise CudaLoweringError("slice_scatter lowering expects a single slice_scatter op")
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("slice_scatter expects inputs [inp, src]")
    inp_name, src_name = (str(x) for x in op.inputs)
    out_name = str(op.output)
    ...
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("slice_scatter lowering currently supports dim=1 only")

    in_shape = _shape_values(intent, inp_name)
    src_shape = _shape_values(intent, src_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(src_shape) != 2 or len(out_shape) != 2:
        raise CudaLoweringError("slice_scatter lowering expects rank-2 tensors")
    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    L = _resolve_dim_int(src_shape[1], bindings, name="L")
    if _resolve_dim_int(src_shape[0], bindings, name="src_M") != M:
        raise CudaLoweringError("slice_scatter src shape mismatch on dim0")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != N:
        raise CudaLoweringError("slice_scatter out shape mismatch")

    start = int((op.attrs or {}).get("start", 0))
    end_raw = int((op.attrs or {}).get("end", N))
    step = int((op.attrs or {}).get("step", 1))
    if step <= 0:
        raise CudaLoweringError("slice_scatter lowering requires step > 0")
    if start < 0:
        start += N
    end = end_raw
    if end < 0:
    ...
    expected_l = (span + step - 1) // step if span > 0 else 0
    if L != expected_l:
        raise CudaLoweringError("slice_scatter src shape mismatch with (start,end,step)")
...
extern "C" __global__ void {intent.name}(
  ...
  for (int k = 0; k < {L}; ++k) {{
    const int dst = {start} + k * {step};
    if ((unsigned)dst < (unsigned)N) {{
      {out_name}[base + (size_t)dst] = {src_name}[(size_t)row * (size_t){L} + (size_t)k];
    }}
  }}
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        ...
        tensor_args=[inp_name, src_name, out_name],
        scalar_args={"M": "i32", "N": "i32"},
        arg_names=[inp_name, src_name, out_name, "M", "N"],
    )
    launch = CudaLaunch(grid=(M, 1, 1), block=(32, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.setdefault("M", int(M))
    ...
    lowered_bindings["SLICE_START"] = int(start)
    lowered_bindings["SLICE_STEP"] = int(step)
    return CudaLoweredKernel(
        kernel_name=intent.name,
        cuda_src=cuda_src,
        io_spec=io_spec,
...
def _kernel_index_add_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "index_add":
        raise CudaLoweringError("index_add lowering expects a single index_add op")
    op = intent.ops[0]
    if len(op.inputs) != 3:
        raise CudaLoweringError("index_add expects inputs [base, index, src]")
    base_name, index_name, src_name = (str(x) for x in op.inputs)
    out_name = str(op.output)
    ...
    if axis < 0:
        axis += 2
    if axis != 0:
        raise CudaLoweringError("index_add lowering currently supports axis=0 only")

    base_shape = _shape_values(intent, base_name)
    idx_shape = _shape_values(intent, index_name)
    src_shape = _shape_values(intent, src_name)
    out_shape = _shape_values(intent, out_name)
    if len(base_shape) != 2 or len(out_shape) != 2 or len(idx_shape) != 1 or len(src_shape) != 2:
        raise CudaLoweringError("index_add lowering expects base/out rank-2, index rank-1, src rank-2")

    M = _resolve_dim_int(base_shape[0], bindings, name="M")
    N = _resolve_dim_int(base_shape[1], bindings, name="N")
    L = _resolve_dim_int(idx_shape[0], bindings, name="L")
    if _resolve_dim_int(out_shape[0], bindings, name="M_out") != M or _resolve_dim_int(out_shape[1], bindings, name="N_out") != N:
        raise CudaLoweringError("index_add output shape mismatch")
    if _resolve_dim_int(src_shape[0], bindings, name="L_src") != L or _resolve_dim_int(src_shape[1], bindings, name="N_src") != N:
        raise CudaLoweringError("index_add src shape mismatch")

    alpha_raw = (op.attrs or {}).get("alpha", 1.0)
    try:
        if isinstance(alpha_raw, (int, float)):
            alpha = float(alpha_raw)
        else:
            alpha = float(bindings.get(str(alpha_raw), alpha_raw))
    except Exception as e:
        raise CudaLoweringError(f"index_add invalid alpha: {alpha_raw!r}") from e
...
#include <stdint.h>
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(
    const float* __restrict__ {base_name},
    const int* __restrict__ {index_name},
    const float* __restrict__ {src_name},
    float* __restrict__ {out_name},
    int M, int N, int L, float alpha) {{
  const int tid = (int)blockIdx.x * (int)blockDim.x + (int)threadIdx.x;
  const int total = M * N;
  if (tid >= total) return;
  const int row = tid / N;
  const int col = tid - row * N;
  float acc = {base_name}[tid];
  for (int i = 0; i < L; ++i) {{
    const int r = {index_name}[i];
    if ((unsigned)r < (unsigned)M && r == row) {{
      acc += alpha * {src_name}[i * N + col];
    }}
  }}
  {out_name}[tid] = acc;
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[base_name, index_name, src_name, out_name],
        scalar_args={"M": "i32", "N": "i32", "L": "i32", "alpha": "f32"},
        arg_names=[base_name, index_name, src_name, out_name, "M", "N", "L", "alpha"],
    )
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"M": int(M), "N": int(N), "L": int(L), "alpha": float(alpha)})
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=[out_name], bindings=lowered_bindings)


def _kernel_index_put_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "index_put":
        raise CudaLoweringError("index_put lowering expects a single index_put op")
    op = intent.ops[0]
    if len(op.inputs) != 4:
        raise CudaLoweringError("index_put expects inputs [base, row_idx, col_idx, values]")
    base_name, row_name, col_name, values_name = (str(x) for x in op.inputs)
    out_name = str(op.output)

    base_shape = _shape_values(intent, base_name)
    row_shape = _shape_values(intent, row_name)
    col_shape = _shape_values(intent, col_name)
    val_shape = _shape_values(intent, values_name)
    out_shape = _shape_values(intent, out_name)
    if len(base_shape) != 2 or len(out_shape) != 2 or len(row_shape) != 1 or len(col_shape) != 1 or len(val_shape) != 1:
        raise CudaLoweringError("index_put lowering expects base/out rank-2 and row/col/values rank-1")

    M = _resolve_dim_int(base_shape[0], bindings, name="M")
    N = _resolve_dim_int(base_shape[1], bindings, name="N")
    L = _resolve_dim_int(row_shape[0], bindings, name="L")
    if _resolve_dim_int(out_shape[0], bindings, name="M_out") != M or _resolve_dim_int(out_shape[1], bindings, name="N_out") != N:
        raise CudaLoweringError("index_put output shape mismatch")
    if _resolve_dim_int(col_shape[0], bindings, name="L_col") != L or _resolve_dim_int(val_shape[0], bindings, name="L_values") != L:
        raise CudaLoweringError("index_put index/value length mismatch")

    accumulate = bool((op.attrs or {}).get("accumulate", False))
    total = int(M) * int(N)
    sched = intent.schedule or ScheduleSketch()
    block_x = _resolve_schedule_int(sched.tile_n, bindings, default=256)
    if block_x <= 0:
    ...
    update_expr = f"{out_name}[tid] = acc + {values_name}[i];" if accumulate else f"{out_name}[tid] = {values_name}[i];"
    cuda_src = f"""
#include <stdint.h>
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(
    const float* __restrict__ {base_name},
    const int* __restrict__ {row_name},
    const int* __restrict__ {col_name},
    const float* __restrict__ {values_name},
    float* __restrict__ {out_name},
    int M, int N, int L) {{
  const int tid = (int)blockIdx.x * (int)blockDim.x + (int)threadIdx.x;
  const int total = M * N;
  ...
  {out_name}[tid] = acc;
  for (int i = 0; i < L; ++i) {{
    const int r = {row_name}[i];
    const int c = {col_name}[i];
    if ((unsigned)r < (unsigned)M && (unsigned)c < (unsigned)N && r == row && c == col) {{
      {update_expr}
      if (!{str(accumulate).lower()}) {{
        acc = {out_name}[tid];
      }}
    }}
  }}
...
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[base_name, row_name, col_name, values_name, out_name],
        scalar_args={"M": "i32", "N": "i32", "L": "i32"},
        arg_names=[base_name, row_name, col_name, values_name, out_name, "M", "N", "L"],
    )
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
...
def _kernel_kron_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ...
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("kron expects inputs [A, B]")
    a_name, b_name = (str(x) for x in op.inputs)
    out_name = str(op.output)
    ...
    if len(a_shape) != 2 or len(b_shape) != 2 or len(out_shape) != 2:
        raise CudaLoweringError("kron lowering currently supports rank-2 tensors")

    M = _resolve_dim_int(a_shape[0], bindings, name="M")
    N = _resolve_dim_int(a_shape[1], bindings, name="N")
    P = _resolve_dim_int(b_shape[0], bindings, name="P")
    Q = _resolve_dim_int(b_shape[1], bindings, name="Q")
    MP = _resolve_dim_int(out_shape[0], bindings, name="MP")
    NQ = _resolve_dim_int(out_shape[1], bindings, name="NQ")
    if MP != M * P or NQ != N * Q:
        raise CudaLoweringError("kron output shape mismatch")
...
extern "C" __global__ __launch_bounds__({block_x}) void {intent.name}(
    ...
    const float* __restrict__ {b_name},
    float* __restrict__ {out_name},
    int M, int N, int P, int Q, int MP, int NQ, int TOTAL) {{
  const int tid = (int)blockIdx.x * (int)blockDim.x + (int)threadIdx.x;
  if (tid >= TOTAL) return;
  const int oi = tid / NQ;
  const int oj = tid - oi * NQ;
  const int i = oi / P;
  const int p = oi - i * P;
  const int j = oj / Q;
  const int q = oj - j * Q;
  const float av = {a_name}[i * N + j];
  const float bv = {b_name}[p * Q + q];
  {out_name}[tid] = av * bv;
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[a_name, b_name, out_name],
        scalar_args={"M": "i32", "N": "i32", "P": "i32", "Q": "i32", "MP": "i32", "NQ": "i32", "TOTAL": "i32"},
        arg_names=[a_name, b_name, out_name, "M", "N", "P", "Q", "MP", "NQ", "TOTAL"],
    )
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"M": int(M), "N": int(N), "P": int(P), "Q": int(Q), "MP": int(MP), "NQ": int(NQ), "TOTAL": int(total)})
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=[out_name], bindings=lowered_bindings)
...
def _kernel_trace_2d_f32_from_mask_pattern(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 6 or [o.op for o in ops] != ["iota", "iota", "eq", "const", "where", "reduce_sum"]:
        raise CudaLoweringError("trace lowering expects iota,iota,eq,const,where,reduce_sum pattern")
    where_op = ops[4]
    red_op = ops[5]
    if len(where_op.inputs) != 3:
        raise CudaLoweringError("trace where op must have inputs [mask, input, zero]")
    input_name = str(where_op.inputs[1])
    if len(red_op.inputs) != 1 or str(red_op.inputs[0]) != str(where_op.output):
        raise CudaLoweringError("trace reduce_sum must consume where output")
    out_name = str(intent.outputs[0]) if intent.outputs else str(red_op.output)
    in_shape = _shape_values(intent, input_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("trace lowering expects rank-2 input")
    if len(out_shape) > 1 or (len(out_shape) == 1 and _resolve_dim_int(out_shape[0], bindings, name="OUT") != 1):
        raise CudaLoweringError("trace lowering expects scalar output")
    if str(intent.tensors[input_name].dtype) != "f32" or str(intent.tensors[out_name].dtype) != "f32":
        raise CudaLoweringError("trace lowering currently supports f32 input/output")
    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
...
extern "C" __global__ void {intent.name}(
    const float* __restrict__ {input_name},
    float* __restrict__ {out_name},
    int M, int N) {{
  if (blockIdx.x != 0 || threadIdx.x != 0) return;
  const int D = (M < N) ? M : N;
  float acc = 0.0f;
  for (int i = 0; i < D; ++i) {{
    acc += {input_name}[(size_t)i * (size_t)N + (size_t)i];
  }}
  {out_name}[0] = acc;
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[input_name, out_name],
        scalar_args={"M": "i32", "N": "i32"},
        arg_names=[input_name, out_name, "M", "N"],
    )
    launch = CudaLaunch(grid=(1, 1, 1), block=(1, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"M": int(M), "N": int(N)})
...
def _kernel_topk_from_sort_gather_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 4 or [o.op for o in ops] != ["sort", "iota", "iota", "gather"]:
        raise CudaLoweringError("topk lowering expects sort,iota,iota,gather pattern")
    sort_op = ops[0]
    gather_op = ops[3]
    if len(sort_op.inputs) != 1:
        raise CudaLoweringError("topk sort op expects one input")
    inp_name = str(sort_op.inputs[0])
    sorted_name = str(sort_op.output)
    gather_inputs = [str(x) for x in gather_op.inputs]
    if len(gather_inputs) != 3 or gather_inputs[0] != sorted_name:
        raise CudaLoweringError("topk gather must consume sorted output and row/col iota indices")
    axis = int((sort_op.attrs or {}).get("axis", -1))
    if axis not in {1, -1}:
        raise CudaLoweringError("topk lowering currently supports axis=1")
    descending = bool((sort_op.attrs or {}).get("descending", True))
    if not descending:
        raise CudaLoweringError("topk lowering currently supports descending=True")
    out_name = str(gather_op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(out_shape) != 2:
        raise CudaLoweringError("topk lowering expects rank-2 input/output")
    if str(intent.tensors[inp_name].dtype) != "f32" or str(intent.tensors[out_name].dtype) != "f32":
        raise CudaLoweringError("topk lowering currently supports f32 tensors")
    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    K = _resolve_dim_int(out_shape[1], bindings, name="K")
    if K <= 0 or K > N:
        raise CudaLoweringError(f"topk lowering requires 0 < K <= N, got K={K}, N={N}")
...
extern "C" __global__ void {intent.name}(
    ...
    int M, int N, int K) {{
  const int row = (int)blockIdx.x;
  if (row >= M || threadIdx.x != 0) return;
  const float* __restrict__ row_ptr = {inp_name} + (size_t)row * (size_t)N;
  float top[{int(K)}];
  ...
  for (int i = 0; i < K; ++i) {{
    {out_name}[(size_t)row * (size_t)K + (size_t)i] = top[i];
  }}
}}
""".lstrip()
    ...
    launch = CudaLaunch(grid=(int(M), 1, 1), block=(1, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
...
def _kernel_unique_1d_i32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 1 or str(ops[0].op) != "unique":
        raise CudaLoweringError("unique lowering expects single unique op")
    op = ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("unique expects one input")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 1 or len(out_shape) != 1:
        raise CudaLoweringError("unique lowering currently supports rank-1 input/output")
    in_dt = str(intent.tensors[inp_name].dtype)
    out_dt = str(intent.tensors[out_name].dtype)
    if in_dt != "i32" or out_dt != "i32":
        raise CudaLoweringError("unique lowering currently supports i32 input/output")
    N = _resolve_dim_int(in_shape[0], bindings, name="N")
    U = _resolve_dim_int(out_shape[0], bindings, name="U")
    cuda_src = f"""
#include <stdint.h>

extern "C" __global__ void {intent.name}(
    const int* __restrict__ {inp_name},
    int* __restrict__ {out_name},
    int N, int U) {{
  if (blockIdx.x != 0 || threadIdx.x != 0) return;
  int ucount = 0;
  for (int i = 0; i < N; ++i) {{
    const int v = {inp_name}[i];
    bool seen = false;
  ...
  const int fill_from = (ucount < U) ? ucount : U;
  for (int i = fill_from; i < U; ++i) {{
    {out_name}[i] = 0;
  }}
  if (U > 1) {{
    for (int i = 1; i < U; ++i) {{
      int v = {out_name}[i];
      int j = i - 1;
      while (j >= 0 && {out_name}[j] > v) {{
        {out_name}[j + 1] = {out_name}[j];
        --j;
      }}
      {out_name}[j + 1] = v;
    }}
...
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, out_name],
        scalar_args={"N": "i32", "U": "i32"},
        arg_names=[inp_name, out_name, "N", "U"],
    )
    launch = CudaLaunch(grid=(1, 1, 1), block=(1, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"N": int(N), "U": int(U)})
    return CudaLoweredKernel(
        kernel_name=intent.name,
        cuda_src=cuda_src,
        io_spec=io_spec,
...
def _kernel_quantile_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "quantile":
        raise CudaLoweringError("quantile lowering expects a single quantile op")
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("quantile expects inputs [inp, q]")
    inp_name = str(op.inputs[0])
    q_name = str(op.inputs[1])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    q_shape = _shape_values(intent, q_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("quantile lowering currently supports rank-2 input")
    if len(q_shape) != 0:
        raise CudaLoweringError("quantile expects scalar q input (rank-0)")
    if len(out_shape) not in {1, 2}:
        raise CudaLoweringError("quantile lowering supports output rank-1 or rank-2 keepdim")

    dim = (op.attrs or {}).get("dim", (op.attrs or {}).get("axis", 1))
    try:
        axis = int(dim)
    except Exception as exc:
        raise CudaLoweringError(f"quantile dim/axis must be int, got {dim!r}") from exc
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("quantile lowering currently supports dim=1 only")
    keepdim = bool((op.attrs or {}).get("keepdim", False))
    interpolation = str((op.attrs or {}).get("interpolation", "linear"))
    if interpolation != "linear":
        raise CudaLoweringError("quantile lowering currently supports interpolation='linear' only")

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if N <= 0:
        raise CudaLoweringError("quantile lowering requires N > 0")
    if N > 4096:
        raise CudaLoweringError(f"quantile lowering currently limits N<=4096, got {N}")
    if len(out_shape) == 1:
        if _resolve_dim_int(out_shape[0], bindings, name="OUT_M") != M:
            raise CudaLoweringError("quantile output shape mismatch: expected [M]")
    else:
        if not keepdim:
            raise CudaLoweringError("rank-2 quantile output requires keepdim=true")
        out_m = _resolve_dim_int(out_shape[0], bindings, name="OUT_M")
        out_n = _resolve_dim_int(out_shape[1], bindings, name="OUT_N")
        if out_m != M or out_n != 1:
            raise CudaLoweringError("quantile output shape mismatch: expected [M,1]")
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
                                         const float* __restrict__ {q_name},
                                         float* __restrict__ {out_name},
                                         {m_param},
                                         {n_param}) {{
  {m_load}
  ...
  if (m >= M) return;
  if ((int)threadIdx.x != 0) return;
  constexpr int N_STACK = {n_stack};
  if (N != N_STACK) return;
  float __q_scalar = {q_name} ? {q_name}[0] : 0.5f;
  __q_scalar = fminf(1.0f, fmaxf(0.0f, __q_scalar));
  float vals[N_STACK];
  const float* row = {inp_name} + (size_t)m * (size_t)N;
  for (int i = 0; i < N_STACK; ++i) vals[i] = row[i];
  ...
  float out_v = vals[0];
  if (N_STACK > 1) {{
    const float pos = __q_scalar * (float)(N_STACK - 1);
    int lo = (int)floorf(pos);
    if (lo < 0) lo = 0;
    if (lo > N_STACK - 1) lo = N_STACK - 1;
    int hi = lo + 1;
    if (hi > N_STACK - 1) hi = N_STACK - 1;
    const float t = pos - (float)lo;
    out_v = vals[lo] + (vals[hi] - vals[lo]) * t;
  }}
  {out_name}[m] = out_v;
}}
""".lstrip()

    tensor_args = [inp_name, q_name, out_name]
    scalar_args: Dict[str, str] = {}
    arg_names = [inp_name, q_name, out_name]
    for dim_name in ["M", "N"]:
        if _is_scalar_tensor(intent, dim_name, dtype="i32"):
            tensor_args.append(dim_name)
            arg_names.append(dim_name)
...
def _kernel_sort_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "sort":
        raise CudaLoweringError("sort lowering expects a single sort op")
    op = intent.ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("sort expects one input tensor")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(out_shape) != 2:
        raise CudaLoweringError("sort lowering currently supports rank-2 tensors")

    axis = int((op.attrs or {}).get("axis", -1))
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("sort lowering currently supports axis=1 only")
    descending = bool((op.attrs or {}).get("descending", False))

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != N:
        raise CudaLoweringError("sort output shape mismatch")
    if N <= 0:
        raise CudaLoweringError("sort lowering requires N > 0")
    if N > 4096:
        raise CudaLoweringError(f"sort lowering currently limits N<=4096, got {N}")
    ...
    n_load = "const int N = N_ptr ? N_ptr[0] : 0;" if n_is_tensor else ""
    n_stack = int(N)
    pick = "vals[N_STACK - 1 - col]" if descending else "vals[col]"
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
  ...
  if (N != N_STACK) return;
  float vals[N_STACK];
  const float* row_ptr = {inp_name} + (size_t)row * (size_t)N;
  for (int col = 0; col < N_STACK; ++col) vals[col] = row_ptr[(size_t)col];
  for (int i = 1; i < N_STACK; ++i) {{
    const float v = vals[i];
    int j = i - 1;
    while (j >= 0 && vals[j] > v) {{
  ...
  float* out_row = {out_name} + (size_t)row * (size_t)N;
  for (int col = 0; col < N_STACK; ++col) out_row[(size_t)col] = {pick};
}}
""".lstrip()
...
def _kernel_stack_2x2d_axis0_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "stack":
        raise CudaLoweringError("stack lowering expects a single stack op")
    op = intent.ops[0]
    if len(op.inputs) != 2:
        raise CudaLoweringError("stack lowering currently supports exactly 2 inputs")
    a_name = str(op.inputs[0])
    b_name = str(op.inputs[1])
    out_name = str(op.output)
    axis = int((op.attrs or {}).get("axis", 0))
    if axis < 0:
        axis += 3
    if axis != 0:
        raise CudaLoweringError("stack lowering currently supports axis=0 only")

    a_shape = _shape_values(intent, a_name)
    b_shape = _shape_values(intent, b_name)
    out_shape = _shape_values(intent, out_name)
    if len(a_shape) != 2 or len(b_shape) != 2 or len(out_shape) != 3:
        raise CudaLoweringError("stack lowering expects rank-2 inputs and rank-3 output")

    M = _resolve_dim_int(a_shape[0], bindings, name="M")
    N = _resolve_dim_int(a_shape[1], bindings, name="N")
    if _resolve_dim_int(b_shape[0], bindings, name="b_M") != M or _resolve_dim_int(b_shape[1], bindings, name="b_N") != N:
        raise CudaLoweringError("stack input shape mismatch")
    if (
        _resolve_dim_int(out_shape[0], bindings, name="out_D0") != 2
        or _resolve_dim_int(out_shape[1], bindings, name="out_M") != M
        or _resolve_dim_int(out_shape[2], bindings, name="out_N") != N
    ):
        raise CudaLoweringError("stack output shape mismatch, expected [2,M,N]")
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {a_name},
                                         const float* __restrict__ {b_name},
                                         float* __restrict__ {out_name},
                                         {m_param},
                                         {n_param}) {{
  ...
  {out_name}[idx] = {a_name}[idx];
  {out_name}[mn + idx] = {b_name}[idx];
}}
""".lstrip()
...
def _kernel_std_2d_axis1_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "std":
        raise CudaLoweringError("std lowering expects a single std op")
    op = intent.ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("std expects one input tensor")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("std lowering currently supports rank-2 input")
    if len(out_shape) not in {1, 2}:
        raise CudaLoweringError("std lowering expects rank-1 or rank-2 output")

    axis = (op.attrs or {}).get("axis", (op.attrs or {}).get("dims", [1]))
    if isinstance(axis, (list, tuple)):
        if len(axis) != 1:
            raise CudaLoweringError("std lowering currently supports one reduction axis")
        axis = int(axis[0])
    else:
        axis = int(axis)
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("std lowering currently supports axis=1 only")
    keepdims = bool((op.attrs or {}).get("keepdims", False))
    correction = int((op.attrs or {}).get("correction", (op.attrs or {}).get("ddof", 0)))

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if len(out_shape) == 1:
        if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M:
            raise CudaLoweringError("std output shape mismatch: expected [M]")
        out_stride = 1
    else:
        if not keepdims:
            raise CudaLoweringError("rank-2 std output requires keepdims=true")
        if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != 1:
            raise CudaLoweringError("std output shape mismatch: expected [M,1]")
        out_stride = 1
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
  ...
  const float* row_ptr = {inp_name} + (size_t)row * (size_t)N;
  double sum = 0.0;
  for (int col = 0; col < N; ++col) sum += (double)row_ptr[(size_t)col];
  const double mean = sum / (double)N;
  double sq = 0.0;
  for (int col = 0; col < N; ++col) {{
    const double d = (double)row_ptr[(size_t)col] - mean;
    sq += d * d;
  }}
  double den = (double)N - (double){int(correction)};
  if (den <= 0.0) {{
    {out_name}[(size_t)row * (size_t){out_stride}] = NAN;
    return;
  }}
  double var = sq / den;
  if (var < 0.0) var = 0.0;
  const float stdv = sqrtf((float)var);
  {out_name}[(size_t)row * (size_t){out_stride}] = stdv;
}}
""".lstrip()
...
def _kernel_var_from_std_mul_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 2 or [o.op for o in ops] != ["std", "mul"]:
        raise CudaLoweringError("var-from-std lowering expects std->mul pattern")
    std_op, mul_op = ops
    if len(std_op.inputs) != 1:
        raise CudaLoweringError("std op must have one input")
    std_out = str(std_op.output)
    if list(mul_op.inputs or []) != [std_out, std_out]:
        raise CudaLoweringError("var-from-std expects mul(std_out, std_out)")

    inp_name = str(std_op.inputs[0])
    out_name = str(mul_op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("var-from-std supports rank-2 input only")
    if len(out_shape) not in {1, 2}:
        raise CudaLoweringError("var-from-std expects rank-1/2 output")

    axis = (std_op.attrs or {}).get("axis", (std_op.attrs or {}).get("dims", [1]))
    if isinstance(axis, (list, tuple)):
        if len(axis) != 1:
            raise CudaLoweringError("var-from-std supports single axis only")
        axis = int(axis[0])
    else:
        axis = int(axis)
    if axis < 0:
        axis += 2
    if axis != 1:
        raise CudaLoweringError("var-from-std supports axis=1 only")
    keepdims = bool((std_op.attrs or {}).get("keepdims", False))
    correction = int((std_op.attrs or {}).get("correction", (std_op.attrs or {}).get("ddof", 0)))

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if len(out_shape) == 1:
        if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M:
            raise CudaLoweringError("var-from-std output shape mismatch: expected [M]")
        out_stride = 1
    else:
        if not keepdims:
            raise CudaLoweringError("rank-2 output requires keepdims=true")
        if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != 1:
            raise CudaLoweringError("var-from-std output shape mismatch: expected [M,1]")
        out_stride = 1
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
  ...
  double var = sq / den;
  if (var < 0.0) var = 0.0;
  {out_name}[(size_t)row * (size_t){out_stride}] = (float)var;
}}
""".lstrip()
...
def _kernel_vector_norm_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 3 or [o.op for o in ops] != ["mul", "reduce_sum", "sqrt"]:
        raise CudaLoweringError("vector_norm lowering expects mul->reduce_sum->sqrt")
    mul_op, reduce_op, sqrt_op = ops
    if len(mul_op.inputs) != 2 or str(mul_op.inputs[0]) != str(mul_op.inputs[1]):
        raise CudaLoweringError("vector_norm expects mul(x, x)")
    if list(reduce_op.inputs or []) != [str(mul_op.output)]:
        raise CudaLoweringError("vector_norm reduce_sum must consume mul output")
    if list(sqrt_op.inputs or []) != [str(reduce_op.output)]:
        raise CudaLoweringError("vector_norm sqrt must consume reduce_sum output")

    inp_name = str(mul_op.inputs[0])
    out_name = str(sqrt_op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2:
        raise CudaLoweringError("vector_norm supports rank-2 input only")
    if len(out_shape) != 1:
        raise CudaLoweringError("vector_norm expects rank-1 output")

    dims = (reduce_op.attrs or {}).get("dims")
    axis = (reduce_op.attrs or {}).get("axis")
    keepdims = bool((reduce_op.attrs or {}).get("keepdims", False))
    if keepdims:
        raise CudaLoweringError("vector_norm lowering currently expects keepdims=false")
    if dims not in ([1], (1,)) and axis not in ([1], 1, "1"):
        raise CudaLoweringError("vector_norm reduction must be axis=1")

    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M:
        raise CudaLoweringError("vector_norm output shape mismatch")
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
  ...
  double sum_sq = 0.0;
  for (int col = 0; col < N; ++col) {{
    const double v = (double)row_ptr[(size_t)col];
    sum_sq += v * v;
  }}
  if (sum_sq < 0.0) sum_sq = 0.0;
  {out_name}[(size_t)row] = sqrtf((float)sum_sq);
}}
""".lstrip()
...
def _kernel_weight_norm_dim0_2d_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    ops = list(intent.ops or [])
    if len(ops) != 6 or [o.op for o in ops] != ["mul", "reduce_sum", "sqrt", "div", "broadcast_in_dim", "mul"]:
        raise CudaLoweringError("weight_norm lowering expects mul->reduce_sum->sqrt->div->broadcast_in_dim->mul")
    sq_op, red_op, sqrt_op, div_op, bcast_op, mul_out_op = ops
    if len(sq_op.inputs) != 2 or str(sq_op.inputs[0]) != str(sq_op.inputs[1]):
        raise CudaLoweringError("weight_norm expects first mul(v, v)")
    v_name = str(sq_op.inputs[0])
    if list(red_op.inputs or []) != [str(sq_op.output)]:
        raise CudaLoweringError("weight_norm reduce_sum must consume squared tensor")
    if list(sqrt_op.inputs or []) != [str(red_op.output)]:
        raise CudaLoweringError("weight_norm sqrt must consume reduce_sum output")
    if len(div_op.inputs) != 2 or str(div_op.inputs[1]) != str(sqrt_op.output):
        raise CudaLoweringError("weight_norm div must be g / norm")
    g_name = str(div_op.inputs[0])
    if list(bcast_op.inputs or []) != [str(div_op.output)]:
        raise CudaLoweringError("weight_norm broadcast_in_dim must consume div output")
    if len(mul_out_op.inputs) != 2:
        raise CudaLoweringError("weight_norm final mul expects 2 inputs")
    out_name = str(mul_out_op.output)

    in_shape = _shape_values(intent, v_name)
    g_shape = _shape_values(intent, g_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 2 or len(g_shape) != 1 or len(out_shape) != 2:
        raise CudaLoweringError("weight_norm lowering expects v[M,N], g[M], out[M,N]")
    M = _resolve_dim_int(in_shape[0], bindings, name="M")
    N = _resolve_dim_int(in_shape[1], bindings, name="N")
    if _resolve_dim_int(g_shape[0], bindings, name="g_M") != M:
        raise CudaLoweringError("weight_norm g shape mismatch")
    if _resolve_dim_int(out_shape[0], bindings, name="out_M") != M or _resolve_dim_int(out_shape[1], bindings, name="out_N") != N:
        raise CudaLoweringError("weight_norm out shape mismatch")

    cuda_src = f"""
#include <math.h>
#include <stdint.h>

extern "C" __global__ void {intent.name}(const float* __restrict__ {v_name},
                                         const float* __restrict__ {g_name},
                                         float* __restrict__ {out_name},
                                         int M, int N) {{
  const int row = (int)blockIdx.x;
  if (row >= M) return;
  ...
  const float* row_ptr = {v_name} + (size_t)row * (size_t)N;
  double sum_sq = 0.0;
  for (int col = 0; col < N; ++col) {{
    const double vv = (double)row_ptr[(size_t)col];
    sum_sq += vv * vv;
  }}
  if (sum_sq < 0.0) sum_sq = 0.0;
  const float norm = sqrtf((float)sum_sq);
  const float scale = (norm > 0.0f) ? ({g_name}[(size_t)row] / norm) : 0.0f;
  float* out_row = {out_name} + (size_t)row * (size_t)N;
  for (int col = 0; col < N; ++col) {{
    out_row[(size_t)col] = row_ptr[(size_t)col] * scale;
  }}
}}
""".lstrip()
    ...
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[v_name, g_name, out_name],
        scalar_args={"M": "i32", "N": "i32"},
        arg_names=[v_name, g_name, out_name, "M", "N"],
    )
    launch = CudaLaunch(grid=(M, 1, 1), block=(32, 1, 1), shared_mem=0)
    lowered_bindings = dict(bindings)
    lowered_bindings.setdefault("M", int(M))
...
def _kernel_upsample_nearest1d_ncl_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "upsample_nearest1d":
        raise CudaLoweringError("upsample_nearest1d lowering expects a single upsample_nearest1d op")
    op = intent.ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("upsample_nearest1d expects 1 input")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 3 or len(out_shape) != 3:
        raise CudaLoweringError("upsample_nearest1d expects rank-3 input/output [N,C,L]")
    N = _resolve_dim_int(in_shape[0], bindings, name="N")
    C = _resolve_dim_int(in_shape[1], bindings, name="C")
    IL = _resolve_dim_int(in_shape[2], bindings, name="IL")
    ON = _resolve_dim_int(out_shape[0], bindings, name="ON")
    OC = _resolve_dim_int(out_shape[1], bindings, name="OC")
    OL = _resolve_dim_int(out_shape[2], bindings, name="OL")
    if ON != N or OC != C:
        raise CudaLoweringError("upsample_nearest1d N/C mismatch")
    if IL <= 0 or OL <= 0:
        raise CudaLoweringError("upsample_nearest1d expects IL/OL > 0")
    total = N * C * OL
    block_x = 256
    grid_x = (total + block_x - 1) // block_x
    cuda_src = f"""
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
                                         float* __restrict__ {out_name},
                                         int N, int C, int IL, int OL, int TOTAL) {{
  const int tid = (int)blockIdx.x * (int)blockDim.x + (int)threadIdx.x;
  if (tid >= TOTAL) return;
  int t = tid;
  const int ol = t % OL; t /= OL;
  const int c = t % C; t /= C;
  const int n = t;
  int il = (int)(((int64_t)ol * (int64_t)IL) / (int64_t)OL);
  if (il >= IL) il = IL - 1;
  const size_t out_idx = (size_t)tid;
  const size_t in_idx = ((size_t)n * (size_t)C + (size_t)c) * (size_t)IL + (size_t)il;
  {out_name}[out_idx] = {inp_name}[in_idx];
}}
""".lstrip()
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"N": int(N), "C": int(C), "IL": int(IL), "OL": int(OL), "TOTAL": int(total)})
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, out_name],
        scalar_args={"N": "i32", "C": "i32", "IL": "i32", "OL": "i32", "TOTAL": "i32"},
        arg_names=[inp_name, out_name, "N", "C", "IL", "OL", "TOTAL"],
    )
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=[out_name], bindings=lowered_bindings)


def _kernel_upsample_nearest2d_nchw_f32(intent: IntentFunction, bindings: Dict[str, int]) -> CudaLoweredKernel:
    if not intent.ops or len(intent.ops) != 1 or intent.ops[0].op != "upsample_nearest2d":
        raise CudaLoweringError("upsample_nearest2d lowering expects a single upsample_nearest2d op")
    op = intent.ops[0]
    if len(op.inputs) != 1:
        raise CudaLoweringError("upsample_nearest2d expects 1 input")
    inp_name = str(op.inputs[0])
    out_name = str(op.output)
    in_shape = _shape_values(intent, inp_name)
    out_shape = _shape_values(intent, out_name)
    if len(in_shape) != 4 or len(out_shape) != 4:
        raise CudaLoweringError("upsample_nearest2d expects rank-4 input/output [N,C,H,W]")
    N = _resolve_dim_int(in_shape[0], bindings, name="N")
    C = _resolve_dim_int(in_shape[1], bindings, name="C")
    ...
    OW = _resolve_dim_int(out_shape[3], bindings, name="OW")
    if ON != N or OC != C:
        raise CudaLoweringError("upsample_nearest2d N/C mismatch")
    if IH <= 0 or IW <= 0 or OH <= 0 or OW <= 0:
        raise CudaLoweringError("upsample_nearest2d expects positive H/W")
    total = N * C * OH * OW
    block_x = 256
    grid_x = (total + block_x - 1) // block_x
    cuda_src = f"""
...
extern "C" __global__ void {intent.name}(const float* __restrict__ {inp_name},
  ...
  const int c = t % C; t /= C;
  const int n = t;
  int ih = (int)(((int64_t)oh * (int64_t)IH) / (int64_t)OH);
  int iw = (int)(((int64_t)ow * (int64_t)IW) / (int64_t)OW);
  if (ih >= IH) ih = IH - 1;
  if (iw >= IW) iw = IW - 1;
  const size_t out_idx = (size_t)tid;
  const size_t in_idx = (((size_t)n * (size_t)C + (size_t)c) * (size_t)IH + (size_t)ih) * (size_t)IW + (size_t)iw;
  {out_name}[out_idx] = {inp_name}[in_idx];
}}
""".lstrip()
    lowered_bindings = dict(bindings)
    lowered_bindings.update({"N": int(N), "C": int(C), "IH": int(IH), "IW": int(IW), "OH": int(OH), "OW": int(OW), "TOTAL": int(total)})
    io_spec = _io_spec_from_args(
        intent,
        tensor_args=[inp_name, out_name],
        scalar_args={"N": "i32", "C": "i32", "IH": "i32", "IW": "i32", "OH": "i32", "OW": "i32", "TOTAL": "i32"},
        arg_names=[inp_name, out_name, "N", "C", "IH", "IW", "OH", "OW", "TOTAL"],
    )
    launch = CudaLaunch(grid=(grid_x, 1, 1), block=(block_x, 1, 1), shared_mem=0)
    return CudaLoweredKernel(kernel_name=intent.name, cuda_src=cuda_src, io_spec=io_spec, launch=launch, output_names=[out_name], bindings=lowered_bindings)
    ...
    *,
    ...
    if intent.ops:
        ...
        if len(intent.ops) == 1:
            ...
            if op0 == "reduce_min":
                op0_node = intent.ops[0]
                out_name = str(op0_node.output)
                out_shape = _shape_values(intent, out_name) if out_name in intent.tensors else []
                dims_raw = (op0_node.attrs or {}).get("dims")
                axis_raw = (op0_node.attrs or {}).get("axis")
                dims_norm: list[int] = []
                if isinstance(dims_raw, (list, tuple)):
                    for d in dims_raw:
                        try:
                            dims_norm.append(int(d))
                        except Exception:
                            continue
                elif dims_raw is not None:
                    try:
                        dims_norm.append(int(dims_raw))
                    except Exception:
                        pass
                elif isinstance(axis_raw, (list, tuple)):
                    for d in axis_raw:
                        try:
                            dims_norm.append(int(d))
                        except Exception:
                    ...
                    try:
                        dims_norm.append(int(axis_raw))
                    except Exception:
                        pass
                if len(out_shape) == 0 and sorted(set(dims_norm)) in ([0, 1], [-2, -1]):
                    return _kernel_reduce_min_2d_all_f32(intent, bindings)
                return _kernel_reduce_min_2d_axis1_f32(intent, bindings)
            if op0 == "gather":
                return _kernel_gather2d_f32(intent, bindings)
            if op0 == "sort":
                return _kernel_sort_2d_f32(intent, bindings)
            if op0 == "stack":
                return _kernel_stack_2x2d_axis0_f32(intent, bindings)
            if op0 == "std":
                return _kernel_std_2d_axis1_f32(intent, bindings)
            if op0 == "scatter":
                return _kernel_scatter_2d_f32(intent, bindings)
            if op0 == "select_scatter":
                return _kernel_select_scatter_2d_f32(intent, bindings)
            if op0 == "slice_scatter":
                return _kernel_slice_scatter_2d_f32(intent, bindings)
            if op0 == "nonzero":
            ...
            if op0 == "quantile":
                return _kernel_quantile_2d_f32(intent, bindings)
            if op0 == "unique":
                return _kernel_unique_1d_i32(intent, bindings)
            if op0 == "avg_pool2d":
                return _kernel_avg_pool2d_nchw_f32(intent, bindings)
            if op0 == "max_pool2d_with_indices":
    ...
    if op_seq and all(str(x) == "max_pool2d_with_indices" for x in op_seq):
        return _kernel_max_pool2d_with_indices_nchw_f32(intent, bindings)
    if len(op_seq) == 3 and sorted(op_seq) == ["const", "div", "reduce_sum"]:
    ...
    if (
        ...
        and len(op_seq) >= 2
        and all(str(x) in {"const", "identity", "broadcast_in_dim", "reduce_min", "where"} for x in op_seq)
    ):
        return _kernel_reduce_min_with_aux_pattern_2d_f32(intent, bindings)
    if op_seq == ["matmul", "cast"]:
        return _kernel_mm_cast_chain_2d_f32(intent, bindings)
    if op_seq == ["cast", "cast", "min", "cast"]:
        return _kernel_minimum_cast_chain_2d_f32(intent, bindings)
    if op_seq == ["iota", "gather"]:
        return _kernel_diag_2d_f32(intent, bindings)
    if op_seq == ["iota", "iota", "eq", "const", "where", "reduce_sum"]:
        return _kernel_trace_2d_f32_from_mask_pattern(intent, bindings)
    if op_seq == ["sort", "iota", "iota", "gather"]:
        return _kernel_topk_from_sort_gather_2d_f32(intent, bindings)
    if op_seq == ["cast", "cast", "mul", "reduce_sum"]:
        return _kernel_dot_1d_f32(intent, bindings)
    if op_seq == ["std", "mul"]:
        return _kernel_var_from_std_mul_2d_f32(intent, bindings)
    if op_seq == ["mul", "reduce_sum", "sqrt"]:
        return _kernel_vector_norm_2d_f32(intent, bindings)
    if op_seq == ["mul", "reduce_sum", "sqrt", "div", "broadcast_in_dim", "mul"]:
        return _kernel_weight_norm_dim0_2d_f32(intent, bindings)
    ...
    if ("rms_norm" in name_l or "rmsnorm" in name_l) and {"reduce_sum", "rsqrt", "broadcast_in_dim", "mul"}.issubset(op_names):
        return _kernel_rms_norm_2d_f32(intent, bindings)
    if (
