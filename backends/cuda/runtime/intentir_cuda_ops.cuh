...
template <int N_ROUNDS>
__device__ __forceinline__ intentir_uint4 intentir_philox_rand4_u32_rounds(uint64_t seed, uint32_t c0) {
  static_assert(N_ROUNDS > 0 && N_ROUNDS <= 10, "intentir_philox_rand4_u32_rounds supports 1..10 rounds");
  uint32_t c1 = 0u, c2 = 0u, c3 = 0u;
  uint32_t k0 = (uint32_t)(seed & 0xFFFFFFFFu);
  uint32_t k1 = (uint32_t)((seed >> 32) & 0xFFFFFFFFu);
  const uint32_t PHILOX_KEY_A = 0x9E3779B9u;
  ...
  return intentir_uint4{c0, c1, c2, c3};
}
