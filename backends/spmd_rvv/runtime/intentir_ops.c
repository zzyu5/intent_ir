...
void intentir_matmul_2d_f32(
  ...
  int use_n_parallel = 0;
  int64_t tn_parallel = tn;
  {
    int thr = omp_get_max_threads();
    if (thr > 1) {
      // Partitioning across N blocks avoids duplicating B reads across threads.
      // Reduce tn until we have >=thr independent N-panels (down to 32 cols).
      while (tn_parallel > 32 && ((N + tn_parallel - 1) / tn_parallel) < (int64_t)thr) tn_parallel >>= 1;
      if (((N + tn_parallel - 1) / tn_parallel) >= (int64_t)thr) use_n_parallel = 1;
...
#pragma omp parallel for schedule(static) if (N >= 64)
      for (int64_t n_base = 0; n_base < N; n_base += tn_parallel) {
        int64_t n_end = n_base + tn_parallel;
        if (n_end > N) n_end = N;
