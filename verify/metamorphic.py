...
def _default_relations(
    ...
    if "upsample" in kernel_name:
        def _transform_affine_shift(
            ...
            for k in ("I", "Input", "input", "input_ptr", "X"):
                if k in out:
                    out[k] = np.asarray(out[k], dtype=np.float32) + 1.0
                    break
            return out
        ...
        def _check_affine_shift(base_out: Dict[str, np.ndarray], tr_out: Dict[str, np.ndarray], bindings: Dict[str, int], *, atol: float, rtol: float) -> Tuple[bool, str]:
            def _pick_y(d: Dict[str, np.ndarray]) -> np.ndarray:
                if "O" in d:
                    return np.asarray(d["O"], dtype=np.float32)
                if "Output" in d:
                    return np.asarray(d["Output"], dtype=np.float32)
                if "out" in d:
                    return np.asarray(d["out"], dtype=np.float32)
                if "output" in d:
                    return np.asarray(d["output"], dtype=np.float32)
            ...
            ok = not bool(np.any(diff > limit))
            if ok:
                return True, "affine shift held"
            max_err = float(np.max(diff))
            return False, f"affine shift violated (max diff {max_err})"

        relations.append(("MR_upsample_affine_shift", _transform_affine_shift, _check_affine_shift))

    if "relu" in kernel_name:
        def _transform_relu_scale(
            inputs: Dict[str, np.ndarray],
            bindings: Dict[str, int],
            rng: np.random.Generator,
        ) -> Dict[str, np.ndarray]:
        ...
        def _check_relu_scale(base_out: Dict[str, np.ndarray], tr_out: Dict[str, np.ndarray], bindings: Dict[str, int], *, atol: float, rtol: float) -> Tuple[bool, str]:
             ...
             ok = bool(np.allclose(y1, expected, atol=1e-3, rtol=1e-3))
             return ok, ("relu homogeneity held" if ok else "relu homogeneity violated")
