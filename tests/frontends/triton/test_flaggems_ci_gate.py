...
def test_ci_gate_coverage_requires_mlir_stage_timing_when_execution_ir_mlir(tmp_path: Path) -> None:
    registry = tmp_path / "registry.json"
    feature = tmp_path / "feature_list.json"
    active_cov = tmp_path / "active_cov.json"
    run_summary = tmp_path / "run_summary.json"
    status_converged = tmp_path / "status_converged.json"
    progress = tmp_path / "progress_log.jsonl"
    handoff = tmp_path / "handoff.md"
    ...
    registry.write_text(json.dumps(registry_payload), encoding="utf-8")
    feature.write_text(
        json.dumps(build_feature_list_payload(registry_payload=registry_payload, source_registry_path=str(registry))),
        encoding="utf-8",
    )
    active_cov.write_text(json.dumps({"schema_version": "flaggems_active_batch_v2", "lane": "coverage", "items": []}), encoding="utf-8")
    # stage_timing without mlir section should fail when execution_ir=mlir.
    stage_timing_json.write_text(
        json.dumps(
            {
                "schema_version": "flaggems_stage_timing_breakdown_v1",
        ...
        json.dumps(
            {
                ...
                "execution_ir": "mlir",
                "stages": [
                    {"stage": "coverage_integrity", "ok": True, "full_coverage_run": True},
                    {"stage": "stage_timing_breakdown", "ok": True, "json_path": str(stage_timing_json)},
                ],
            }
        ),
