...
def _run_gate(tmp_path: Path, *, include_next_focus: bool) -> subprocess.CompletedProcess[str]:
    ...
    status_converged.write_text(
        json.dumps({"entries": [{"semantic_op": "angle", "status": "blocked_backend", "reason_code": "missing_e2e_spec"}]}),
        encoding="utf-8",
    )
    progress.write_text(
        json.dumps(
        ...
        json.dumps(
            {
                ...
                "scoped_entries": [
                    {"semantic_op": "angle", "status": "blocked_backend", "reason_code": "diff_fail"},
                ],
            }
        ),
        encoding="utf-8",
