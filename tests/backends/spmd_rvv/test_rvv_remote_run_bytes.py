...
def test_rvv_execution_plan_requires_hardcut_evidence(tmp_path: Path) -> None:
    mod = _load_module()
    contract_path, payload = _write_contract(
        tmp_path,
...
def test_rvv_execution_plan_requires_contract_payload() -> None:
    mod = _load_module()
    try:
        _ = mod._resolve_rvv_execution_plan(
        ...
        assert "requires mlir backend contract" in str(e)
    else:
